...
describe Simp::Cli::Passgen::CommandCommon do
  ...
  describe '#legacy_passgen?' do
    it 'returns true for old simplib' do
      expect(@passgen_cmd.legacy_passgen?('3.17.0')).to be(true)
    end

    it 'returns false for new simplib' do
      expect(@passgen_cmd.legacy_passgen?('4.0.1')).to be(false)
    end
  end

  describe '#translate_bool' do
    it 'translates true to enabled' do
      expect(@passgen_cmd.translate_bool(true)).to eq('enabled')
    end

    it 'translates false to disabled' do
