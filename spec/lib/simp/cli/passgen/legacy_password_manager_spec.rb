...
describe Simp::Cli::Passgen::LegacyPasswordManager do
  before :each do
    ...
    @manager = described_class.new(@env)
  end
  ...
  describe '#remove_password' do
    ...
    it 'removes password, backup, and salt files for name specified env' do
      ...
      expect(File.exist?(@name1_file)).to be false
      expect(File.exist?(@name1_salt_file)).to be false
      expect(File.exist?(@name1_backup_file)).to be false
      expect(File.exist?(@name1_backup_salt_file)).to be false
      expect(File.exist?(@name2_file)).to be true
      expect(File.exist?(@name2_salt_file)).to be true
      expect(File.exist?(@name3_file)).to be true
      expect(File.exist?(@name3_salt_file)).to be true
      expect(File.exist?(@name3_backup_file)).to be true
      expect(File.exist?(@name3_backup_salt_file)).to be true

      @manager.remove_password('production_name2')

      expect(File.exist?(@name2_file)).to be false
      expect(File.exist?(@name2_salt_file)).to be false
    end
    ...
    it 'removes password, backup, and salt files in specified password dir' do
      ...
      expect(File.exist?(File.join(@alt_password_dir, 'env1_name4'))).to be false
      expect(File.exist?(File.join(@alt_password_dir, 'env1_name4.salt'))).to be false
      expect(File.exist?(File.join(@alt_password_dir, 'env1_name4.last'))).to be false
      expect(File.exist?(File.join(@alt_password_dir, 'env1_name4.salt.last'))).to be false
    end
