...
describe Simp::Cli::Utils do
  describe '.show_wait_spinner' do
    it 'returns result of block' do
      result = described_class.show_wait_spinner do
        sleep 1
        'block result'
      end
    ...
    it 'kills spinning thread when block raises' do
      base_num_threads = Thread.list.count { |thread| thread.status == 'run' }
      error = nil
      begin
        described_class.show_wait_spinner do
          sleep 1
      ...
      current_num_threads = Thread.list.count { |thread| thread.status == 'run' }
  ...
  describe '.validate_password' do
    it 'validates good passwords' do
      expect { described_class.validate_password 'A=V3ry=Go0d=P@ssw0r!' }
        .not_to raise_error
    end

    it 'raises an PasswordError on short passwords' do
      expect { described_class.validate_password 'a@1X' }.to raise_error(Simp::Cli::PasswordError)
    end

    it 'raises an PasswordError on simple passwords' do
      expect { described_class.validate_password 'aaaaaaaaaaaaaaa' }.to raise_error(Simp::Cli::PasswordError)
    end
  end
  ...
  describe '.validate_password_with_cracklib' do
    ...
    it 'raises an PasswordError on simple passwords' do
      expect { described_class.validate_password '012345678901234' }.to raise_error(Simp::Cli::PasswordError)
    end
  end
  ...
  describe '.generate_password' do
    ...
    context 'with defaults' do
      it 'returns a password of the default length' do
        expect(described_class.generate_password.size).to \
          eq Simp::Cli::Utils::DEFAULT_PASSWORD_LENGTH
      end

      it 'returns a password with default and safe special characters' do
      ...
      it 'returns a password that does not start/end with a special char' do
        expect(described_class.generate_password).not_to match(%r{^[@%\-_+=~]|[@%\-_+=~]$})
      end
    end

    context 'with custom settings that validate' do
      it 'returns a password of the specified length' do
        expect(described_class.generate_password(73).size).to eq 73
      end

      it 'returns a password that contains all special characters if complexity is 2' do
        result = described_class.generate_password(32, 2)
    ...
    context 'with custom settings that do not validate' do
      it 'returns a password that contains no special chars if complexity is 0' do
        result = described_class.generate_password(32, 0, false, 10, false)
        expect(result).to match(%r{(#{default_chars.join('|')})})
        expect(result).not_to match(%r{(#{safe_special_chars.join('|')})})
        expect(result).not_to match(%r{(#{unsafe_special_chars.join('|')})})
      end

      it 'returns a password that only contains "safe" special chars if complexity is 1 and complex_only is true' do
        result = described_class.generate_password(32, 1, true, 10, false)
        expect(result.length).to be(32)
      ...
      it 'returns a password that only contains all special chars if complexity is 2 and complex_only is true' do
        result = described_class.generate_password(32, 2, true, 10, false)
        expect(result.length).to be(32)
        expect(result).not_to match(%r{(#{default_chars.join('|')})})
        expect(result).to match(%r{(#{unsafe_special_chars.join('|')})})
      end
    ...
    context 'errors' do
      it 'fails when password generation times out' do
        ...
        expect { described_class.generate_password(8, 2, true, 20, true) }
  ...
  describe '.yes_or_no' do
    ...
    it "when default_yes=true, prompts, accepts default of 'yes' and " \
      ...
      expect(described_class.yes_or_no('Remove backups', true))
        .to be true
    ...
    it "when default_yes=false, prompts, accepts default of 'no' and " \
      ...
      @input << "\n"
      @input.rewind
      expect(described_class.yes_or_no('Remove backups', false))
        .to be false
