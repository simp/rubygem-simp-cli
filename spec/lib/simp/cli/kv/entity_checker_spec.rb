...
describe Simp::Cli::Kv::EntityChecker do
  ...
  before :each do
    ...
    @checker = described_class.new(@env, @backend)
  end
  ...
  describe '#exists' do
    it 'returns true when key/folder exists in the environment' do
      ...
      expect(@checker.exists(key, false)).to be true
    end
    ...
    it 'returns true when global key/folder exists' do
      ...
      expect(@checker.exists(key, true)).to be true
    end
    ...
    it 'returns false when key/folder does not exist in the environment' do
      ...
      expect(@checker.exists(key, false)).to be false
    end
    ...
    it 'returns false when global key/folder does not exist' do
      ...
      expect(@checker.exists(key, true)).to be false
    end
  ...
  describe '#get_exists' do
    it 'returns true when manifest apply succeeds and log includes exists string' do
    ...
    it 'returns false when manifest apply succeeds and log does not include exists string' do
      err_msg = 'Error: Evaluation Error: Error while evaluating a Function ' \
    ...
    it 'fails when #apply_manifest_with_spawn fails' do
      allow(Simp::Cli::ApplyUtils).to receive(:apply_manifest_with_spawn)
