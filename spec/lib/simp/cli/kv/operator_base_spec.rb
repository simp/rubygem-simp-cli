...
describe Simp::Cli::Kv::OperatorBase do
  ...
  before :each do
    ...
    @base = described_class.new(@env, @backend)
  end

  describe '#apply_options' do
    it 'returns options without :fail_filter when failure_message unset' do
    ...
    it 'returns options with :fail_filter when failure_message set' do
      title = 'some operation'
  ...
  describe '#full_store_path' do
    it 'returns path within environment when not global' do
      expect(@base.full_store_path('key_a', false)).to eq("/environments/#{@env}/key_a")
    end

    it 'returns global path when global' do
      expect(@base.full_store_path('key_a', true)).to eq('/globals/key_a')
    end

    it 'returns remove extraneous slashes' do
  ...
  describe '#normalize_list' do
    it 'encodes values for all keys with binary values' do
