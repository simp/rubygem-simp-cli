...
describe Simp::Cli::Puppetfile::Skeleton do
  ...
  describe '.to_puppetfile' do
    ...
    context 'when no environment is specified' do
      context 'when no local modules exist in the module path' do
        it 'generates skeleton Puppetfile' do
          expect(described_class.new.to_puppetfile).to eq expected_skeleton_only
        end
      end

      context 'when local modules exist in the module path' do
        it 'generates skeleton Puppetfile without local modules' do
          expect(described_class.new.to_puppetfile).to eq expected_skeleton_only
        end
      end
    end
    ...
    context 'when an environment is specified' do
      ...
      context 'when no modules exist in the module path' do
        it 'generates skeleton Puppetfile' do
          ...
          expect(described_class.new(@puppet_env, '/does/not/exist').to_puppetfile).to eq expected_skeleton_only
        end
      end
      ...
      context 'when all local modules exist in the module path' do
        it 'generates skeleton Puppetfile with local modules' do
          ...
          expect(described_class.new(@puppet_env, '/does/not/exist').to_puppetfile).to eq expected
        end
      end
    end
    ...
    context "when the 'git' command could not be found" do
      it 'fails' do
        ...
        expect { described_class.new(@puppet_env, '/does/not/exist').to_puppetfile }.to raise_error(
          Simp::Cli::ProcessingError, %r{Could not find 'git' command}
        )
      end
    end
  end
  ...
  describe '.load_metadata' do
    ...
    it 'returns metadata Hash when metadata.json is valid' do
    ...
    it 'returns nil Hash when metadata.json does not exist' do
      generator = described_class.new
    ...
    it 'returns nil Hash when metadata.json is malformed' do
      ...
      expect { generator.send(:load_metadata, module_dir) }.to output(
        %r{Ignoring local module #{Regexp.escape(module_dir)}},
      ).to_stderr
    end

    it "returns nil Hash when metadata.json is missing top-level 'name' key" do
