...
describe Simp::Cli::Environment::Env do
  ...
  describe '#run_command' do
    ...
    it 'returns true when command succeeeds' do
      command = "ls #{__FILE__}"
      expect(@env.run_command(command)[:status]).to be true
    ...
    it 'returns false when command fails and ignore_failure is false' do
      ...
      expect(@env.run_command(command)[:status]).to be false
      expect(@env.run_command(command)[:stdout]).to eq ''
      expect(@env.run_command(command)[:stderr]).to match(%r{ls: cannot access.*/some/missing/path1.*: No such file or directory})
    end

    it 'returns true when command fails and ignore_failure is true' do
      command = 'ls /some/missing/path1 /some/missing/path2'
      expect(@env.run_command(command, true)[:status]).to be true
      expect(@env.run_command(command)[:stdout]).to eq ''
      expect(@env.run_command(command)[:stderr]).to match(%r{ls: cannot access.*/some/missing/path1.*: No such file or directory})
    end
    # rubocop:enable RSpec/MultipleExpectations
  ...
  describe '#execute' do
    ...
    it 'returns true when command succeeeds' do
      command = "ls #{__FILE__}"
      expect(@env.execute(command)).to be true
    end

    it 'returns false when command fails and ignore_failure is false' do
      command = 'ls /some/missing/path1 /some/missing/path2'
      expect(@env.execute(command)).to be false
    end

    it 'returns true when command fails and ignore_failure is true' do
      command = 'ls /some/missing/path1 /some/missing/path2'
      expect(@env.execute(command, true)).to be true
    end
  end
