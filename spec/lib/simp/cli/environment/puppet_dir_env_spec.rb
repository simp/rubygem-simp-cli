  ...
  context 'with methods' do
    ...
    describe '#create_environment_from_skeleton' do
      before(:each) do
        ...
        allow(FileUtils).to receive(:mkdir_p).with(env_dir, mode: 0o755)
    ...
    describe '#puppetfile_generate' do
      ...
      it 'generates Puppetfile.simp and Puppetfile when none exist' do
      ...
      it 'generates Puppetfile.simp, only, when Puppetfile exists' do
      ...
      it 'generates Puppetfile.simp only when both exist' do
        File.open(@puppetfile, 'w') { |file| file.puts 'old Puppetfile content' }
