...
describe Simp::Cli::Commands::Environment::New do
  describe '#run' do
    ...
    let(:default_writable_hash_opts) do
      {
        enabled: true,
        strategy: :skeleton,
        backend: :directory
    ...
    let(:default_secondary_hash_opts) do
      {
        enabled: true,
        strategy: :skeleton,
        backend: :directory,
    ...
    let(:default_puppet_hash_opts) do
      {
        enabled: true,
        strategy: :skeleton,
      ...
      context "with #{strategy} strategy" do
        ...
        test_params.each do |args, expected_hash|
          ...
          context "with args '#{full_args.join(' ')}'" do
            subject(:simp_env_new) { proc { described_class.new.run(cli_args) } }
