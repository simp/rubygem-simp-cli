  ...
  describe ".run" do
    ...
    it "does not apply actions when user is not root" do
      ...
      expected_lines = [
        ...
        "#{fmt_begin}(skipping apply [**user is not root**] )#{fmt_end} yaml::file_writer"
      ]
    ...
    it "prints a summary of actions" do
      ...
      expect(summary).to match /Digest algorithm adjustment for FIPS/m
      expect(summary).to match /Configuration of network interfaces skipped/m
      expect(summary).to match /Setting of hostname skipped/m
      expect(summary).to match /Setting of GRUB password skipped/m
      expect(summary).to match /Certificate setup for SIMP skipped/m
      expect(summary).to match /Rename of puppet.your.domain.yaml template to <host>.yaml skipped/m
      expect(summary).to match /Update to YUM repos and settings in <host>.yaml skipped/m
      ...
      expect(summary).to match /Creation of #{@output_file} applied/m
    end
