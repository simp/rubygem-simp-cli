...
describe Simp::Cli::Config::SimpPuppetEnvHelper do
  before :each do
    ...
    @env_helper = described_class.new(@env, @start_time)
  end
  ...
  describe '#back_up_puppet_environment' do
    ...
    it 'backups an puppet env when the backup parent dir does not exist' do
    ...
    it 'backups an puppet env when the backup parent dir does exist' do
      puppet_env_dir = File.join(@tmp_dir, 'environments', 'production')
      FileUtils.mkdir_p(puppet_env_dir)
      expected_parent_dir = File.join(@tmp_dir, 'environments.bak')
    ...
    it 'does nothing when the puppet env to back up does not exist' do
      puppet_env_dir = File.join(@tmp_dir, 'environments', 'production')
  ...
  describe '#create' do
    ...
    it 'returns new env info after create' do
      skip('Tested in simp-core acceptance tests')
    end

    it 'backups existing Puppet environment' do
  ...
  describe '#env_info' do
    ...
    it 'returns Hash of info about the SIMP omni-environment' do
      allow(@env_helper).to receive_messages(get_system_puppet_info: system_puppet_info, get_puppet_env_datadir: datadir)
    ...
    [:missing, :empty, :present, :invalid].each do |pup_env_code|
      [:missing, :present, :invalid].each do |sec_env_code|
        ...
        it "returns #{status_code} for pup env #{pup_env_code} and sec env #{sec_env_code}" do
          ...
          allow(@env_helper).to receive_messages(puppet_env_status: [pup_env_code, pup_env_detail], secondary_env_status: [sec_env_code, sec_env_detail])
  ...
  describe '#puppet_env_status' do
    it 'returns :missing when the env does not exist' do
      allow(@env_helper).to receive_messages(get_system_puppet_info: @system_puppet_info, get_puppet_env_datadir: nil)
  ...
  describe '#secondary_env_status' do
    it 'returns :missing when the env does not exist' do
      allow(@env_helper).to receive_messages(get_system_puppet_info: @system_puppet_info, get_puppet_env_datadir: @puppet_env_datadir)
