...
describe Simp::Cli::Config::Item::SimpOptionsNTPServers do
  ...
  describe '#validate' do
    it 'validates array with good hosts' do
      expect(@ci.validate(['pool.ntp.org'])).to be true
      expect(@ci.validate(['192.168.1.1'])).to be true
      expect(@ci.validate(['192.168.1.1', 'pool.ntp.org'])).to be true
    ...
    it "doesn't validate array with bad hosts" do
      ...
      expect(@ci.validate(['pool.ntp.org-'])).to be false
      expect(@ci.validate(['192.168.1.1.'])).to be false
