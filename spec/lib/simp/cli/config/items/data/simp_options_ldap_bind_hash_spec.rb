...
describe Simp::Cli::Config::Item::SimpOptionsLdapBindHash do
  before :each do
    ...
    item = Simp::Cli::Config::Item::SimpOptionsLdapBindPw.new
  ...
  describe '#validate' do
    it 'validates OpenLDAP-format SHA-1 algorithm (FIPS 160-1) password hash' do
      expect(@ci.validate('{SSHA}zxOLQEdncCJTMObl5s+y1N/Ydh3vsi6s')).to be false
    end

    it 'fails validation when the LDAP bind password does not validate against the hash' do
      expect(@ci.validate('{SSHA}Y6x92VpatHf9G6yMiktUYTrA/3SxUFm')).to be false
    end
  end
