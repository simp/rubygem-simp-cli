...
describe Simp::Cli::Config::Item::SimpOptionsTrustedNets do
  ...
  describe '#validate' do
    it 'validates array of good cidr nets' do
      expect(@ci.validate(['10.0.71.0/24'])).to be true
      expect(@ci.validate(['1.2.3.0/24'])).to be true
      expect(@ci.validate(['10.10.10.0/16', '192.168.1.0/23'])).to be true
      # crazy, but valid CIDR (equiv to 33,554,432 class C networks!)
      expect(@ci.validate(['1.2.3.0/0'])).to be true
    end
    ...
    it "doesn't validate array with bad cidr nets" do
      ...
      expect(@ci.validate([nil])).to be false
      expect(@ci.validate(['1.2.3.999/24'])).to be false
      expect(@ci.validate(['1.2.3.999/24/24'])).to be false
      expect(@ci.validate(['1.2.3.999/z'])).to be false
      expect(@ci.validate(['1.2.3.0/24', '192.168.1.0/99'])).to be false
    end
  ...
  def create_prior_items(ip, netmask)
    items = {}
    _ip = Simp::Cli::Config::Item::CliNetworkIPAddress.new
    _ip.value = ip
    items[_ip.key] = _ip
    _netmask = Simp::Cli::Config::Item::CliNetworkNetmask.new
