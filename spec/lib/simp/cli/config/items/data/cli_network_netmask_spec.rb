...
describe Simp::Cli::Config::Item::CliNetworkNetmask do
  ...
  describe '#validate' do
    it 'validates netmasks' do
      expect(@ci.validate('255.255.255.0')).to be true
      expect(@ci.validate('255.254.0.0')).to be true
      expect(@ci.validate('192.0.0.0')).to be true
    end

    it "doesn't validate bad netmasks" do
      expect(@ci.validate('999.999.999.999')).to be false
      expect(@ci.validate('255.999.0.0')).to be false
      expect(@ci.validate('255.0.255.0')).to be false
      expect(@ci.validate('0.255.0.0')).to be false
