...
describe Simp::Cli::Config::Item::SimpOptionsLdapBaseDn do
  ...
  describe '#recommended_value' do
    ...
    it 'returns no value when cli::is_simp_ldap_server is true and hostname is not set' do
      item = Simp::Cli::Config::Item::CliIsSimpLdapServer.new
      item.value = true
      @ci.config_items[item.key] = item
      expect(@ci.recommended_value).to be_nil
    ...
    it 'returns value based on hostname when cli::is_simp_ldap_server is true and hostname is set' do
      item = Simp::Cli::Config::Item::CliIsSimpLdapServer.new
      item.value = true
      @ci.config_items[item.key] = item
  ...
  describe '#validate' do
    it 'validates ldap_base_dns' do
      expect(@ci.validate('dc=tasty,dc=bacon')).to be true
    end

    it "doesn't validate bad ldap_base_dns" do
      expect(@ci.validate('cn=hostAuth,ou=Hosts,dc=tasty,dc=bacon')).to be false
