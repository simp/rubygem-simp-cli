  ...
  describe '#apply' do
    context 'when keydist directory exists' do
      ...
      context 'when cert generation is not required' do
        it 'reports :unnecessary status' do
          ...
          expect(@ci.apply_summary).to match(%r{Interim certificate generation for 'puppet.testing.fqdn' unnecessary:\n    Certificates already exist}m)
        end
      end
    end
    ...
    context 'when keydist directory does not exist' do
      before :each do
        ...
        @dirs_adjusted.each { |dir| FileUtils.chmod(0o700, dir) }
      ...
      it 'creates dir tree, fixes perms, generates certs and reports :succeeded status on success' do
        @ci.apply
        expect(@ci.applied_status).to eq :succeeded
        @dirs_adjusted.each { |dir| expect(File.stat(dir).mode & 0o777).to eq 0o755 }
        ...
        expected_mode = if File.stat(File.join(@secondary_env, 'site_files')).mode.allbits?(0o070)
                          0o770
                        else
                          0o750
                        end
        ...
        [
          ...
          expect(File.stat(dir).mode & 0o777).to eq expected_mode
        end
