...
describe Simp::Cli::Config::Item::YumRepositories do
  ...
  context "not in SIMP directory structure" do
    ...
    describe '#apply' do
      ...
      it "returns false when yum OS/REL/ARCH dir does not exist" do
        result = @ci.apply
        expect( result ).to eq false
      end

      it "returns false when yum OS/REL/ARCH dir cannot be accessed" do
        FileUtils.mkdir_p @yum_dist_dir
      ...
      it "returns false when yum Updates dir cannot be created due to permissions" do
        FileUtils.mkdir_p @yum_dist_dir
      ...
      it "returns false when yum Updates dir cannot be created because a file named Updates exists" do
        FileUtils.mkdir_p @yum_dist_dir
      ...
      it "returns false when yum Updates dir cannot be accessed" do
      ...
      it "returns false when yum.repos.d does not exist" do
        FileUtils.mkdir_p @yum_dist_dir
      ...
      it "returns false when yum.repos.d cannot be accessed" do
        FileUtils.mkdir_p @yum_dist_dir
      ...
      it "returns false when yum.repos.d is not a directory" do
        FileUtils.mkdir_p @yum_dist_dir
        FileUtils.rm_rf @tmp_repos_d
      ...
      it "returns false when puppet.your.domain.yaml does not exist" do
        FileUtils.mkdir_p @yum_dist_dir
  ...
  describe "#apply_summary" do
    it 'reports not attempted status when #safe_apply not called' do
      ci = Simp::Cli::Config::Item::YumRepositories.new
      expect(ci.apply_summary).to eq 'Update to YUM repos and settings in <host>.yaml not attempted'
    end
  end
