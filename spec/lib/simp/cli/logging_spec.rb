...
class MyLogTesterA
  ...
  def self.use_logger
    logger.trace("#{inspect}: this is an already-formatted trace message".magenta)
    logger.debug("#{inspect}: this is an unformatted debug message with a {")
    logger.info("#{inspect}: this is a single-part, formatted info message with a }", [:BOLD, :RED])
    logger.notice("#{inspect}: this is an already-formatted notice message".bold.green)
    logger.warn("#{inspect}: this is a", nil, ' multi-part ', [:BOLD], 'formatted warn message')
    logger.error("#{inspect}: this is a message that does not end in a newline when sent to the console... ")
    logger.error("#{inspect}: continuation first line")
    logger.fatal("#{inspect}: this is a", [], ' message ', [:RED],
                 'that does not end in a newline when sent to the console... ')
    logger.fatal("#{inspect}: continuation second line")
  end
end
...
describe Simp::Cli::Logging do
  before :each do
    ...
    described_class.logger.open_logfile(@log_file)
  ...
  describe 'log to console and file allowing error and above log messages' do
    before :each do
      ...
      described_class.logger.levels(:error, :error)
    end
...
EOM
        actual_file_output = normalize_logfile(File.read(@log_file))
...
EOM
  ...
  describe 'log to console and file allowing all log messages' do
    before :each do
      described_class.logger.levels(:trace, :trace)
    end
...
EOM
  ...
  describe 'count_down' do
    it 'prints generic countdown message to the console' do
    ...
    it 'prints customized countdown message to the console' do
      # exact match doesn't work in TravisCI
      expected = %r{\.\.\.Resuming in 2 seconds.*\.\.\.Resuming in 1 seconds.*\.\.\.Resuming in 0 seconds\n}
