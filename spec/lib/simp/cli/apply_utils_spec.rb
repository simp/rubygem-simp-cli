...
describe Simp::Cli::ApplyUtils do
  describe '.apply_manifest_with_spawn' do
    ...
    let(:manifest_with_fail_message) do
      <<~EOM
    ...
    context 'without logger' do
      it 'returns apply result when apply from default options succeeds' do
        cmd_regex = %r{^#{Regexp.escape(apply_cmd_prefix)} .*apply_manifest.pp$}

        allow(Simp::Cli::ExecUtils).to receive(:run_command)
          .with(cmd_regex).and_return(success_result)

        expect(described_class.apply_manifest_with_spawn(manifest))
          .to eq(success_result)
      end

      it 'returns apply result when apply with custom options succeeds' do
        cmd_regex = %r{^#{Regexp.escape(sg_cmd_prefix)} .*apply_manifest.pp'}
        allow(Simp::Cli::ExecUtils).to receive(:run_command)
          .with(cmd_regex).and_return(success_result)
      ...
      it 'returns apply result when apply fails and :fail option is false' do
        ...
        opts = { :fail => false }
        expect(described_class.apply_manifest_with_spawn(manifest, opts))
          .to eq(fail_result)
      end
      ...
      it 'fails with :fail_filter message when apply fails and stderr matches that message' do
        ...
        expect {
          described_class.apply_manifest_with_spawn(
    ...
    context 'with logger' do
      it 'returns apply result when apply succeeds and logs' do
        ...
        expect(described_class.apply_manifest_with_spawn(manifest, {}, logger))
          .to eq(success_result)
        ...
        expected_regex = %r{Executing: #{Regexp.escape(apply_cmd_prefix)} .*apply_manifest.pp$}
      ...
      it 'fails when apply fails and logs' do
        ...
        expected_regex = %r{Executing: #{Regexp.escape(sg_cmd_prefix)} .*apply_manifest.pp'$}
        expect(logger.messages[:debug][1][0]).to match(expected_regex)
  ...
  describe '.load_yaml' do
    ...
    context 'without logger' do
      it 'returns Hash for valid YAML file' do
        ...
        yaml = described_class.load_yaml(file, 'password info')
      ...
      it 'fails when the file does not exist' do
        expect { described_class.load_yaml('oops', 'password list') }
          .to raise_error(Simp::Cli::ProcessingError,
                          %r{Failed to load password list YAML:\n<<< Error: No such file or directory}m)
      end

      it 'fails when YAML file cannot be parsed' do
        file = File.join(files_dir, 'bad.yaml')
        expected_regex = %r{Failed to load password info YAML:\n<<< YAML Content:\n#{File.read(file)}\n<<< Error: }m
        expect { described_class.load_yaml(file, 'password info') }
    ...
    context 'with logger' do
      it 'returns Hash for valid YAML file' do
        file = File.join(files_dir, 'good.yaml')
        logger = TestUtils::MockLogger.new
        yaml = described_class.load_yaml(file, 'info', logger)
      ...
      it 'fails when the file does not exist' do
        logger = TestUtils::MockLogger.new
        expect {
          described_class.load_yaml('oops', 'password list', logger)
        }
          .to raise_error(Simp::Cli::ProcessingError,
                          %r{Failed to load password list YAML:\n<<< Error: No such file or directory}m)
      ...
      it 'fails when YAML file cannot be parsed' do
        ...
        expected_regex = %r{Failed to load password info YAML:\n<<< YAML Content:\n#{File.read(file)}\n<<< Error: }m
        expect {
          described_class.load_yaml(file, 'password info', logger)
        }
          .to raise_error(Simp::Cli::ProcessingError, expected_regex)
