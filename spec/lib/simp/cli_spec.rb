...
EOM
      ...
      it 'outputs general usage when invalid command specified' do
        expect { @result = Simp::Cli.start(['oops']) }.to output(%r{oops is not a recognized command}).to_stderr
        expect(@result).to be @failure_status
      end

      it 'outputs bootstrap usage when bootstrap help specified' do
        expect { @result = Simp::Cli.start(['bootstrap', '-h']) }.to output(%r{=== The SIMP Bootstrap Tool ===}m).to_stdout
        expect(@result).to be @success_status
      end

      it 'outputs config usage when config help specified' do
        expect { @result = Simp::Cli.start(['config', '-h']) }.to output(%r{=== The SIMP Configuration Tool ===}m).to_stdout
        expect(@result).to be @success_status
      end

      it 'outputs doc usage when doc help specified' do
        expect { @result = Simp::Cli.start(['doc', '-h']) }.to output(%r{=== The SIMP Doc Tool ===}m).to_stdout
        expect(@result).to be @success_status
      end

      it 'outputs environment usage when environment help specified' do
        expect { @result = Simp::Cli.start(['environment', '-h']) }.to output(%r{=== The SIMP Environment Tool ===}m).to_stdout
        expect(@result).to be @success_status
      end

      it 'outputs passgen usage when passgen help specified' do
        expect { @result = Simp::Cli.start(['passgen', '-h']) }.to output(%r{=== The SIMP Password Tool ===}m).to_stdout
        expect(@result).to be @success_status
      end

      it 'outputs puppetfile usage when puppetfile help specified' do
        expect { @result = Simp::Cli.start(['puppetfile', '-h']) }.to output(%r{=== The SIMP Puppetfile Tool ===}m).to_stdout
        expect(@result).to be @success_status
      end

      it 'outputs version usage when version help specified' do
        expect { @result = Simp::Cli.start(['version', '-h']) }.to output(%r{Display the current version of SIMP}m).to_stdout
        expect(@result).to be @success_status
      end
    end
    ...
    describe 'fails when invalid command option specified' do
      it 'fails to run bootstrap' do
        expect { @result = Simp::Cli.start(['bootstrap', '--oops-bootstrap']) }.to output(
          %r{'bootstrap' command options error: invalid option: --oops-bootstrap},
        ).to_stderr
      ...
      it 'fails to run config' do
        expect { @result = Simp::Cli.start(['config', '--oops-config']) }.to output(
          %r{'config' command options error: invalid option: --oops-config},
        ).to_stderr
      ...
      it 'fails to run passgen' do
        expect { @result = Simp::Cli.start(['passgen', '--oops-passgen']) }.to output(
          %r{'passgen' command options error: invalid option: --oops-passgen},
        ).to_stderr
