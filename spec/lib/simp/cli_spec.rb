...
describe "Simp::Cli" do
  ...
  describe "Simp::Cli::start" do
    describe "help" do
      ...
      it "outputs cleancerts usage when cleancerts help specified" do
        expect{ @result = Simp::Cli.start(['help','cleancerts']) }.to output(/=== The SIMP CleanCerts Tool ===/m).to_stdout
        expect( @result ).to be @success_status
        expect{ @result = Simp::Cli.start(['cleancerts', '-h']) }.to output(/=== The SIMP CleanCerts Tool ===/m).to_stdout
        expect( @result ).to be @success_status
      end
      ...
      it "outputs puppeteval usage when puppeteval help specified" do
        expect{ @result = Simp::Cli.start(['help','puppeteval']) }.to output(/=== The SIMP PuppetEval Tool ===/m).to_stdout
        expect( @result ).to be @success_status
      end
    
      it "outputs runpuppet usage when runpuppet help specified" do
        expect{ @result = Simp::Cli.start(['help','runpuppet']) }.to output(/=== The SIMP RunPuppet Tool ===/m).to_stdout
        expect( @result ).to be @success_status
        expect{ @result = Simp::Cli.start(['runpuppet', '-h']) }.to output(/=== The SIMP RunPuppet Tool ===/m).to_stdout
        expect( @result ).to be @success_status
      end
    end
    ...
    describe "fails when invalid command option specified" do
      ...
      it "fails to run cleancerts" do
        expect{ @result = Simp::Cli.start(['cleancerts', '--oops-cleancerts']) }.to output(
          /'cleancerts' command options error: invalid option: --oops-cleancerts/).to_stderr
      ...
      it "fails to run runpuppet" do
        expect{ @result = Simp::Cli.start(['runpuppet', '--oops-runpuppet']) }.to output(
          /'runpuppet' command options error: invalid option: --oops-runpuppet/).to_stderr
