...
shared_examples 'simp config operation' do |host, options|
  ...
  let(:config_opts) do
    ...
    config << if opts[:set_grub_password]
                "'simp_grub::password=#{grub_pwd_hash}'"
              else
  ...
  if opts[:priv_user] && !opts[:priv_user][:exists]
    it 'ensures local priv user does not yet exist as a precondition' do
      on(host, "userdel -r #{opts[:priv_user][:name]}", :accept_all_exit_codes => true)
    end
  end

  it "runs `simp config` to configure server for bootstrap #{opts[:description]}" do
    on(host, "#{opts[:env_vars].join(' ')} simp config #{config_opts.join(' ')}")
  end

  it "creates the #{opts[:puppet_env]} Puppet environment" do
    expect(directory_exists_on(host, puppet_env_dir)).to be true
  end

  it 'creates a pair of Puppetfiles configured for SIMP' do
    ...
    modules.each do |name|
      ...
      repo_name = result.stdout.match(%r{(\w+-\w*)})[1]
      on(host, "grep '/usr/share/simp/git/puppet_modules/#{repo_name}.git' #{puppet_env_dir}/Puppetfile.simp")
    end
  end

  it 'creates a environment.conf with secondary env in modulepath' do
  ...
  it 'creates a hiera.yaml.conf that matches enviroment-skeleton' do
  ...
  it 'populates modules dir with modules from local git repos' do
    modules.each do |name|
      expect(directory_exists_on(host, "#{puppet_env_dir}/modules/#{name}")).to be true
    end
  end

  it 'creates a simp_config_settings.yaml global hiera file' do
    ...
    if opts[:ldap_server]
      ...
      expected['simp::sssd::client::ldap_server_type'] = if os_release > '7'
                                                           '389ds'
                                                         else
                                                           'plain'
                                                         end
    ...
    elsif opts[:scenario] == 'poss'
      expected['simp_options::sssd'] = if opts[:sssd]
                                         true
                                       else
                                         false
                                       end
    ...
    normalized_exp = expected.delete_if { |_key, value| value == 'SKIP' }
  ...
  it 'creates a <SIMP server fqdn>.yaml hiera file' do
  ...
  it "sets $simp_scenario to #{opts[:scenario]} in site.pp" do
    ...
    expect(actual).to match(%r{^\$simp_scenario\s*=\s*'#{opts[:scenario]}'})
  end

  it "creates a #{opts[:puppet_env]} secondary environment" do
  ...
  it 'creates cacerts and host cert files in the secondary env' do
  ...
  it 'minimallies configure Puppet' do
  ...
  if opts[:priv_user] && !opts[:priv_user][:exists]
    it "creates privileged user '#{opts[:priv_user][:name]}'" do
    ...
    it "is able to login via ssh as '#{opts[:priv_user][:name]}' using password before bootstrap" do
  ...
  if opts[:priv_user] && opts[:priv_user][:has_keys]
    it "copies privileged user '#{opts[:priv_user][:name]}' keys to /etc/ssh/local_keys" do
      keys_file = "/etc/ssh/local_keys/#{opts[:priv_user][:name]}"
      expect(file_exists_on(host, keys_file)).to be true
    end
  end

  it 'ensures puppet server entry is in /etc/hosts' do
    file_contents_on(host, '/etc/hosts')
