...
shared_examples 'simp asset manual install' do |server|
  context 'asset installation from fixtures staging dir' do
    ...
    it 'ensures packages required by assets are installed' do
      [
        'git',
        'rsync',
      ].each do |package|
    ...
    it 'installs simp-adapter as done by its RPM' do
    ...
    it 'creates skeleton dir' do
      on(server, "mkdir -p #{skeleton_dir}")
    end

    it 'installs simp-environment-skeleton as done by its RPM' do
    ...
    it 'installs simp-rsync-skeleton as done by its RPM' do
      on(server, "cp -r #{asset_staging_dir}/rsync_data/rsync #{skeleton_dir}")
      on(server, "chmod -R g+rX,o-rwx #{skeleton_dir}/rsync")
    end

    it "builds SIMP's selinux contexts as done by simp-selinux-policy RPM" do
      server.install_package('yum-utils')
      # NOTE: two points about this test:
    ...
    it "installs SIMP's selinux contexts as done by simp-selinux-policy RPM" do
      file_install_cmd = [
        'install -p -m 644 -D',
    ...
    it 'installs simp-cli and highline gems in /usr/share/simp/ruby' do
    ...
    it "installs 'simp' script similar to that done by the rubygem-simp-cli RPM" do
      simp_script = <<~EOM
        #!/bin/bash
