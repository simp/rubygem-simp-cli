...
shared_examples 'configure puppet env' do |host, env|
  it "sets the puppet environment to #{env} on #{host}" do
  ...
  it "reloads the puppetserver on #{host} to pick up the changes" do
  ...
  it "waits for the reloaded puppetserver to be available on #{host}" do
