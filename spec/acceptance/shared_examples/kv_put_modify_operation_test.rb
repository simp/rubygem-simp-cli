...
shared_examples 'kv put modify operation test' do |host, env, backend_opt|
  ...
  it "stores new key info for top-level #{env} env keys" do
  ...
  it "stores new key info for sub-folder #{env} env keys" do
  ...
  it 'stores new key info for top-level global keys' do
  ...
  it 'stores new key info for sub-folder global keys' do
    create_remote_file(host, infile, updated_keys_sub_global.to_json)
    updated_keys_sub_global.keys
    store_cmd = "umask 0077; simp kv put -i #{infile} --force -e #{env} " \
                "--global #{backend_opt}"
    on(host, store_cmd)
