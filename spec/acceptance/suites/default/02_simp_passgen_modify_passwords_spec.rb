...
def validate_password(password, options)
  ...
  unsafe_special_chars = ((' '..'/').to_a + ('['..'`').to_a + ('{'..'~').to_a).map { |x|
    Regexp.escape(x)
  } - safe_special_chars
  ...
  expect(password).to match(%r{(#{chars_to_match.join('|')})})
  return if chars_to_not_match.empty?

  expect(password).not_to match(%r{(#{chars_to_not_match.join('|')})})
end
    ...
    hosts.each do |host|
      context 'Password auto-regeneration' do
        context 'using defaults' do
          ...
          if env == 'new_simplib_simpkv_passgen'
            it "regens passwords with current length+complexity+complex_only in #{env}" do
              ...
              names.each do |name, options|
                ...
                new_password = set_result.match(%r{.*new password: (.*)})[1]
            ...
            it "regens passwords with current length in #{env}" do
              saved_latest_passwords.clear
              names.each do |name, options|
                cmd = "simp passgen set #{name} -e #{env} --auto-gen"
                set_result = on(host, cmd).stdout
          ...
          it "lists current and previous passwords in top folder in #{env}" do
            names.each_key do |name|
              ...
              expect(list_result).to match(%r{Current:  #{Regexp.escape(curr_value)}}m)
              expect(list_result).to match(%r{Previous: #{Regexp.escape(prev_value)}}m)
            end
          end

          if env == 'new_simplib_simpkv_passgen'
            ['app1', 'app2', 'app3'].each do |folder|
              it "lists current and previous passwords for #{folder}/ names in #{env}" do
                names.each_key do |name|
      ...
      context 'Password input by user' do
        ...
        it 'accepts user entered password' do
      ...
      context "Applying changes in #{env}" do
        ...
        context 'puppet agent run' do
          ...
          [
            ...
            it "updates file #{file} with latest password" do
          ...
          if env == 'new_simplib_simpkv_passgen'
            ['app1', 'app2', 'app3'].each do |folder|
              [
                ...
                it "updates folder key file #{file} with latest password" do
