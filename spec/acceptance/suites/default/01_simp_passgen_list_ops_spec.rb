...
describe 'simp passgen list operations' do
  context 'environment list' do
    hosts.each do |host|
      it 'onlies list environments containing simp-simplib module' do
        result = on(host, 'simp passgen envs').stdout
        expect(result).to match(%r{old_simplib})
        expect(result).to match(%r{new_simplib_legacy_passgen})
        expect(result).to match(%r{new_simplib_simpkv_passgen})
        expect(result).not_to match(%r{production})
      end
    end
  end
    ...
    [
      ...
      hosts.each do |host|
        ...
        context "name list for #{env} environment" do
          ...
          if env == 'new_simplib_simpkv_passgen'
            ['app1', 'app2', 'app3'].each do |folder|
              it "lists #{folder} folder names from passgen_test" do
                cmd = "simp passgen list -e #{env} --folder #{folder}"
                result = on(host, cmd).stdout
                names.each do |name|
                  expect(result).to match(%r{sub_#{name}})
                end
              end
            end
          end
        ...
        context "password info list for #{env} environment" do
          it 'lists basic password info for top folder names from passgen_test' do
            names.each do |name|
              list_result = on(host, "simp passgen show #{name} -e #{env}").stdout
              value = on(host, "cat /var/passgen_test/#{env}-#{name}").stdout
              expect(list_result).to match(%r{#{Regexp.escape(value)}})
            end
          end

          it 'lists detailed password info for top folder names from passgen_test' do
            names.each do |name|
          ...
          if env == 'new_simplib_simpkv_passgen'
            ['app1', 'app2', 'app3'].each do |folder|
              it "lists basic password info for #{folder}/ names from passgen_test" do
                names.each do |name|
              ...
              it "lists detailed password info for #{folder}/ folder names from passgen_test" do
                names.each do |name|
