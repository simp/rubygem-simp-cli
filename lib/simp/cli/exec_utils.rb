...
module Simp::Cli::ExecUtils
  ...
  def self.run_command(command, ignore_failure = false, logger = nil)
    ...
    raise Simp::Cli::InvalidSpawnError, command if command.include? '|'
    ...
    if ps.success?
      { :status => true, :stdout => stdout, :stderr => stderr }
    else
      logger&.error("\n[#{command}] failed with exit status #{ps.exitstatus}:".red)
      stderr.split("\n").each do |line|
        logger&.error(((' ' * 2) + line).red)
      end
      { :status => false, :stdout => stdout, :stderr => stderr }
    end
  end
  ...
  def self.execute(command, ignore_failure = false, logger = nil)
    run_command(command, ignore_failure, logger)[:status]
  end
end
