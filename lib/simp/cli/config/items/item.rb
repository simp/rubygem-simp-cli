...
module Simp::Cli::Config
  class Item
    ...
    attr_reader :key, :description, :data_type, :fact, :puppet_env_info

    attr_accessor :value, :start_time, :skip_query, :skip_yaml, :silent, :config_items, :next_items_tree
    ...
    def value_required?
      return false if (@data_type == :none) || (@data_type == :global_class)
    ...
    def to_yaml_s(include_auto_warning = false)
      raise InternalError, "@key is empty for #{self.class}" if @key.to_s.empty?
    ...
    def print_summary
      ...
      log_params = []
      log_params += ["(#{@alt_source})", [:CYAN, :BOLD]] unless @alt_source.nil?
    ...
    def determine_value_without_override(allow_queries, force_defaults)
      if !allow_queries && !force_defaults
        err_msg = "FATAL: No answer found for '#{@key}'"
        raise Simp::Cli::Config::ValidationError, err_msg
      else
        if force_defaults
    ...
    def determine_value_with_override(allow_queries, force_defaults)
      if validate(@value)
        ...
        # - The user presets the LDAP BIND password, but doesn't know to remove
        #   or fix the Item containing the hash of the password, because when run
    ...
    def query_ask
      ...
      ask("<%= color('#{query_prompt.gsub("'", "\\\\'")}', WHITE, BOLD) %>: ",
        ...
        q.responses[:not_valid] += err_msg.to_s.red
    ...
    def validate(_x)
      ...
      raise InternalError, "#{self.class}.validate() not implemented!"
    end
    ...
    def verify_item_present(key)
      return if @config_items.key?(key)
