...
module Simp::Cli::Config
  class Item::ConfigureNetworkAction < ActionItem
    ...
    def apply
      ...
      cmd = %(FACTER_ipaddress=XXX #{@puppet_apply_cmd} -e "network::eth{'#{@interface}': bootproto => '#{bootproto}', onboot => true)
      ...
      if bootproto == 'none'
        ...
        resolv_domain = hostname.split('.')[1..].join('.')
        cmd += %(, )
        cmd += %(ipaddr => '#{ipaddress}', )
        cmd += %(netmask => '#{netmask}', )
        cmd += %(gateway => '#{gateway}' } )
        cmd += %(class{ 'resolv': )
        cmd += %(resolv_domain => '#{resolv_domain}', )
        cmd += %(servers => #{format_puppet_array(dns_servers)}, )
        cmd += %(search => #{format_puppet_array(dns_search)}, )
        cmd += %(named_autoconf => false, )
      end
      cmd += %(}")
    ...
    def apply_summary
      "Configuration of #{@interface || 'a'} network interface #{@applied_status}"
    end
