...
class Simp::Cli::Config::ItemListFactory
  ...
  def process(answers_hash = {}, items_yaml = nil)
    ...
    begin
      ...
      logger.error('<' * 80)
      raise Simp::Cli::Config::InternalError, 'invalid Items list YAML'
    end
  ...
  def build_item_queue(item_queue, items)
    ...
    unless items.empty?
      ...
      if item.is_a? String
        ...
        answers_tree = {}
        item.values.first.each do |answer, values|
  ...
  def create_safety_writer_item
    return unless (file = @options[:safety_save_file])
    ...
    if @options[:force_defaults]
      ...
      if @options[:clean_session]
        ...
        unless agree(question) { |q| q.default = 'yes' }
          raise Simp::Cli::ProcessingError, 'Exiting: User terminated processing prior to questionnaire.'
        end
      end
      item.query
      item.print_summary
